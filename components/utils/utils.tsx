...
export function BriefcaseIcon(props : any) {
  ...
  export function TrendingUpIcon(props : any) {
    return (
      <svg
        {...props}
        xmlns="http://www.w3.org/2000/svg"
  ...
  export function LinkItem({ href, text, icon: Icon, imageSrc }: LinkItemProps): JSX.Element {
    return (
      <Link href={href} className="flex items-center gap-4 px-2.5 text-muted-foreground hover:text-foreground" prefetch={false}>
        {Icon && <Icon className="h-5 w-5" />} 
        {imageSrc && <img src={imageSrc} alt={text} className="h-5 w-5" />} 
        {text}
      </Link>
    );
  }